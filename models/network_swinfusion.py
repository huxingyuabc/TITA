...
from models.OAF import OAFBlock
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.relation_judger = nn.Sequential(
            Mlp(dim * 2, dim, dim), torch.nn.Softmax(dim=-1)
        )
    ...
    def forward(self, x, y, x_size):
        ...
        new_y = []
        for bs in range(B):
            judger_input10 = torch.cat(
                [y[bs].unsqueeze(0), x[bs].unsqueeze(0)], dim=-1
            )
            judger_input01 = torch.cat(
                [x[bs].unsqueeze(0), y[bs].unsqueeze(0)], dim=-1
            )

            relation_score10 = self.relation_judger(judger_input10)
            relation_score01 = self.relation_judger(judger_input01)

            ## 1. 0_to_1 cross attn and skip connect
            q0 = x[bs].unsqueeze(0)

            noise_k = self.k_noise.weight[0] + q0
            noise_v = self.v_noise.weight[0] + q0

            k0 = torch.cat([noise_k-torch.mul(q0, relation_score01),
                           noise_v+torch.mul(q0, relation_score01)], dim=0)
            v0 = torch.cat([q0, y[bs].unsqueeze(0)], dim=0)

            x_, w0 = self.cross_attn_0_to_1(q0, k0, v0)
            new_x.append(x[bs] + x_[0].squeeze(0))

            ## 2. 1_to_0 cross attn and skip connect
            q1 = y[bs].unsqueeze(0)

            noise_k = self.k_noise.weight[1] + q1
            noise_v = self.v_noise.weight[1] + q1

            k1 = torch.cat([noise_k-torch.mul(q1, relation_score10),
                           noise_v+torch.mul(q1, relation_score10)], dim=0)
            v1 = torch.cat([q1, x[bs].unsqueeze(0)], dim=0)

            y_, w1 = self.cross_attn_1_to_0(q1, k1, v1)
            new_y.append(y[bs] + y_[0].squeeze(0))
...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 embed_dim=96, Ex_depths=[4], Fusion_depths=[4, 4], Re_depths=[4],
                 Ex_num_heads=[6], Fusion_num_heads=[6, 6], Re_num_heads=[6],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=1, img_range=1., upsampler='', resi_connection='oaf',
                 **kwargs):
        ...
        ######################################### OAF Module definition #########################################
        elif resi_connection == 'oaf':
            # dynamic weight generator
            self.weight_gen = nn.Sequential(
                nn.AdaptiveAvgPool2d((1, 1)),  # GAP
                nn.Flatten(),
                Mlp(embed_dim * 2, embed_dim, 6),
                nn.Softmax(dim=-1)
            )

            self.fafb_A = OAFBlock(in_channels=embed_dim, compressed_channels=embed_dim)
            self.fafb_B = OAFBlock(in_channels=embed_dim, compressed_channels=embed_dim)
        #########################################################################################################
    ...
    def forward_features_Fusion(self, x, y):
        ...
        ########################################## OAF Module forward ###########################################
        new_x = 0
        feat_list_A = self.fafb_A(x)
        feat_list_B = self.fafb_B(y)
        weight = self.weight_gen(torch.cat([x,y], 1))
        for i, (feat_A, feat_B) in enumerate(zip(feat_list_A, feat_list_B)):
            new_x += weight[:, i].view(-1, 1, 1, 1) * feat_A
            new_x += weight[:, i + 3].view(-1, 1, 1, 1) * feat_B
        x = new_x
